window.DriverListView = Backbone.View.extend({
    ...
    render: function () {
        ...
        for (var i = startPos; i < endPos; i++) {
            $('.thumbnails', this.el).append(new DriverListItemView({model: wines[i]}).render().el);
        }

        $(this.el).append(new Paginator({model: this.model, page: this.options.page,uname:"drivers"}).render().el);
